        ...
        <section aria-labelledby="capability-grid-title" className="mb-10 px-4 sm:px-6 md:mb-14">
            <div className="mx-auto max-w-6xl">
                <div className="mb-6 md:mb-7">
                    <h2 id="capability-grid-title" className="text-2xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-3xl">
                        ...
                        return (
                            <li key={capability.title} className="flex">
                                <Link
                                    ...
                                    className="group flex min-h-64 w-full flex-col rounded-[26px] border border-slate-300 bg-slate-50 p-6 shadow-[0_18px_45px_-36px_rgba(15,23,42,0.34)] transition hover:-translate-y-0.5 hover:border-slate-400 hover:bg-white hover:shadow-[0_22px_55px_-34px_rgba(15,23,42,0.38)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200 focus-visible:ring-offset-2 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600 dark:hover:bg-slate-800 dark:hover:shadow-[0_22px_55px_-34px_rgba(0,0,0,0.9)] dark:focus-visible:ring-blue-300 dark:focus-visible:ring-offset-slate-950 md:p-7"
                                >
                                    <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 text-blue-950 ring-1 ring-slate-200 dark:bg-blue-500/15 dark:text-blue-200 dark:ring-blue-400/20">
                                        <Icon aria-hidden="true" className="h-5 w-5" />
                                    </div>
                                    <h3 id={`${capabilityId}-title`} className="text-xl font-semibold text-slate-950 dark:text-white">
                                        {capability.title}
                                    </h3>
                                    <p id={`${capabilityId}-description`} className="mt-3 text-base leading-7 text-slate-600 dark:text-slate-300">
                                        {capability.description}
                                    </p>
                                    <div aria-hidden="true" className="mt-auto inline-flex items-center gap-2 pt-7 text-sm font-semibold text-blue-900 dark:text-blue-300">
                                        Åpne
