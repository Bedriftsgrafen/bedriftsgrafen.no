...
import { formatLargeNumber, normalizeSearchQuery } from '../../utils/formatters'
...
export function HeroSearch() {
    ...
    const eyebrowText = stats
        ? `INNSIKT I ${formatLargeNumber(stats.total_companies)} NORSKE VIRKSOMHETER`
    ...
    return (
        <section className="relative z-20 mb-10 pt-8 md:mb-14 md:pt-14">
            <div className="absolute inset-x-0 top-0 -z-10 h-105 bg-[radial-gradient(circle_at_top,rgba(37,99,235,0.12),transparent_58%)]" />
            <div className="mx-auto max-w-5xl px-4 sm:px-6">
                <div className="overflow-visible rounded-4xl border border-slate-300 bg-slate-50/95 px-5 py-8 shadow-[0_28px_70px_-42px_rgba(15,23,42,0.44)] backdrop-blur sm:px-8 md:px-10 md:py-10">
                    <div className="mx-auto max-w-3xl text-center">
                        <p className="mx-auto mb-4 max-w-2xl text-xs font-semibold uppercase tracking-[0.28em] text-blue-800 sm:text-sm">
                            {eyebrowText}
                        </p>
                        <h1 id="hero-title" className="text-balance text-4xl font-bold tracking-tight text-slate-950 sm:text-5xl md:text-[3.75rem] md:leading-[1.04]">
                    ...
                    <div className="relative z-30 mx-auto mt-8 max-w-3xl rounded-[28px] border border-slate-300 bg-slate-100/80 p-3 shadow-[0_20px_45px_-34px_rgba(15,23,42,0.38)] sm:mt-10 sm:p-4 md:p-5">
                        <div
                            role="group"
                            aria-labelledby={SEARCH_MODE_LABEL_ID}
                            className="mx-auto mb-4 grid w-full max-w-sm grid-cols-2 rounded-full bg-white p-1.5 ring-1 ring-slate-200 sm:flex sm:w-auto sm:max-w-fit"
                        >
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200 focus-visible:ring-offset-2 ${
                                    searchMode === 'company'
                                        ? 'bg-blue-900 text-white shadow-[0_10px_20px_-14px_rgba(30,58,138,0.9)]'
                        ...
                        <form
                            ...
                            className="rounded-2xl border border-slate-300 bg-slate-50 p-2.5 shadow-[0_16px_36px_-28px_rgba(15,23,42,0.32)] focus-within:border-blue-800 focus-within:ring-4 focus-within:ring-blue-100 sm:p-3 md:p-4"
                        >
                            ...
                            <div className="relative flex flex-col gap-3 md:flex-row">
                                <div className="relative flex-1">
                                    <input
                                        ...
                                        className="w-full rounded-xl border-0 bg-transparent px-4 py-3.5 text-base font-medium text-slate-950 outline-none placeholder:text-slate-400 md:text-[17px]"
                                    />
                                    ...
                                    {showCompanyDropdown && (
                                        <div
                                            ...
                                            aria-live="polite"
                                            className="absolute left-0 right-0 top-full z-100 mt-2 overflow-hidden rounded-2xl border border-slate-300 bg-white text-slate-900 shadow-[0_26px_56px_-26px_rgba(15,23,42,0.42)]"
                                        >
                                            {companySearchLoading ? (
                                                <div role="status" className="p-4 text-center text-sm text-slate-500">Søker...</div>
                                            ) : (
                                                <ul role="list" className="max-h-72 overflow-y-auto">
                                                    {companyResults?.map((company) => (
                                                        <li key={company.orgnr}>
                                                            <Link
                                                                ...
                                                                className="flex items-center justify-between gap-3 border-b border-slate-100 p-4 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200 focus-visible:ring-inset last:border-b-0"
                                                            >
                                                                <div className="flex min-w-0 items-center gap-3">
                                                                    ...
                                                                    <div className="min-w-0">
                                                                        <div className="truncate font-semibold text-slate-900">{company.navn || company.orgnr}</div>
                                                    ...
                                                    <Link
                                                        ...
                                                        className="flex items-center justify-center gap-2 border-t border-slate-100 p-3 text-sm font-semibold text-blue-700 transition-colors hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-200 focus-visible:ring-inset"
                                                    >
                                                        Se alle resultater
                                                            ...
                                                            >
                                                                <div className="flex items-center gap-3">
                                                                    ...
                                                                    <div>
                                                                        ...
                                                                        {person.birthdate && (
                                                                            <div className="text-xs text-slate-500">Fødselsår: {person.birthdate.slice(0, 4)}</div>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                                <div aria-hidden="true" className="rounded-full bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600">
                            ...
                            {companyValidationMessage && (
                                <p className="mt-2 px-2 text-sm font-medium text-slate-600" role="status">
                                    {companyValidationMessage}
                                </p>
                            )}
                        </form>
