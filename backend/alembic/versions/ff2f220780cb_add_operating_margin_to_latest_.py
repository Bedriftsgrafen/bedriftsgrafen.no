...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX idx_latest_financials_op_profit_desc ON latest_financials (driftsresultat DESC NULLS LAST);"
    )
    ...
    op.execute(
        "CREATE INDEX idx_latest_financials_margin_desc ON latest_financials (operating_margin DESC NULLS LAST);"
    )
