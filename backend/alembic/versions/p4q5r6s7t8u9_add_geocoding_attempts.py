...
def upgrade() -> None:
    ...
    op.add_column("bedrifter", sa.Column("geocoding_attempts", sa.Integer(), nullable=False, server_default="0"))
    # Create index for efficient filtering of companies needing geocoding
    op.create_index("ix_bedrifter_geocoding_attempts", "bedrifter", ["geocoding_attempts"], unique=False)
