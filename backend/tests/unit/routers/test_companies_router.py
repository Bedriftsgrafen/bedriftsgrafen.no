...
def test_get_companies_success(client, mock_company_service):
    # Arrange
    mock_company_service.get_companies.return_value = [
        CompanyBase(**MOCK_COMPANY_DATA)
    ]
...
def test_fetch_company_data(client, mock_company_service):
    ...
    expected_resp = FetchCompanyResponse(
        orgnr=MOCK_ORGNR,
        company_fetched=True,
        financials_fetched=1,
        errors=[]
    )
...
def test_search_companies(client, mock_company_service):
    # Arrange
    mock_company_service.search_companies.return_value = [
        CompanyBase(**MOCK_COMPANY_DATA)
    ]
