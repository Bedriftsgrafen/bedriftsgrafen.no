...
async def test_get_statistics_performance(mock_db_session):
    ...
    with patch.object(service.company_repo, 'count', new_callable=AsyncMock), \
...
async def test_get_statistics_fallback_consistency(mock_db_session):
    ...
    with patch("services.company_service.search_cache.get", new_callable=AsyncMock) as mock_cache_get, \
         patch("services.company_service.search_cache.set", new_callable=AsyncMock):
        mock_cache_get.return_value = None

        # Financial stats and others should still work
        service.accounting_repo.get_aggregated_stats = AsyncMock(return_value={
            "total_revenue": 0, "total_ebitda": 0, "profitable_percentage": 0, 
            "solid_company_percentage": 0, "avg_operating_margin": 0
        })
