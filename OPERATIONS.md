...
## Dependency Management (Backend: pip-tools)

We use `pip-tools` to manage deterministic builds. 
- `requirements.in`: High-level dependencies with loose constraints.
- `requirements.txt`: Fully pinned, autogenerated dependency graph.

### Update Workflow
1. Edit `backend/requirements.in`.
2. Generate locked file:
   ```bash
   # ⚠️ IMPORTANT: pip-tools 7.x requires pip < 26.0
   backend/.venv/bin/pip install "pip<26.0"
   
   # Compile dependencies
   backend/.venv/bin/pip-compile backend/requirements.in --output-file=backend/requirements.txt
   ```
3. Sync environment:
   ```bash
   backend/.venv/bin/pip-sync backend/requirements.txt
   ```

### Check Outdated Packages
```bash
# Detailed check for new versions
backend/.venv/bin/pip-compile --upgrade --dry-run backend/requirements.in
```
